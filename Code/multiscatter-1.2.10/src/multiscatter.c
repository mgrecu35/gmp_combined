    ...
    ms_real *bscat_air_out)   /* Measured backscatter of air, m-1 sr-1 */
{
  ...
  if (config->small_angle_algorithm != MS_SINGLE_AND_SMALL_ANGLE_NONE) {
	...
	if (config->small_angle_algorithm == MS_SMALL_ANGLE_PVC_FAST_LAG) {
	  if (config->small_angle_lag) {
	    free(config->small_angle_lag);
	  }
	  config->small_angle_lag = malloc(sizeof(ms_real)*n);
	  if (!config->small_angle_lag) {
	    fprintf(stderr, "Error allocating memory for small angle lag\n");
	    return MS_MEMORY_ALLOCATION_ERROR;
	  }
	  status = ms_fast_small_angle_lag(n,
		   instrument_1fov, surface, 
		   range, radius, ext, ext_bscat_ratio, ext_air,
		   droplet_fraction, pristine_ice_fraction, 
		   bscat_out + ifov*m,
		   bscat_air_out + ifov*m*require_bscat_air, 
		   config->small_angle_lag);
	}
	  ...
	  int i;
	  for (i = 0; i < n; i++) {
	    bscat_AD[i] = bscat_air_AD[i] = radius_AD[i] = ext_AD[i]
	      = ext_bscat_ratio_AD[i] = ext_air_AD[i]
	      = droplet_fraction_AD[i] = pristine_ice_fraction_AD[i] = 0.0;
	  }
	  fprintf(stderr, "*** Testing normal code part of adjoint REMOVE IN PRODUCTION CODE ***\n");
	  status = ms_fast_small_angle_AD(n, 
			  ...
			  radius_AD, ext_AD, ext_bscat_ratio_AD,
			  ext_air_AD, droplet_fraction_AD, 
			  pristine_ice_fraction_AD);

#endif

	}
	else if (config->small_angle_algorithm == MS_SMALL_ANGLE_PVC_ORIGINAL) {
	  status = ms_small_angle(n,
  ...
  */
  ...
  ms_real g_AD[n];
  for (i = 0; i < n; i++) {
    bscat_AD[i] = ext_AD[i] = g_AD[i] = ssa_AD[i] = radius_AD[i] = 0.0;
  }
  fprintf(stderr, "*** Testing normal code part of wide-angle adjoint REMOVE IN PRODUCTION CODE ***\n");
  return ms_wide_angle_AD(n, m,
			  config, instrument, surface, range, radius,
			  ext, ssa, g, ext_air, ssa_air,
			  bscat_out, 
			  bscat_AD, radius_AD,ext_AD, ssa_AD, g_AD);
#endif
}

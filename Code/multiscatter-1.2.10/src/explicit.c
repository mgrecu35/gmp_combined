...
scatter(int n, int start_gate, int order, int norder, int output_dist,
	...
	const ms_real *fs_fraction, const ms_real *theta2, 
	const ms_real *rho2range2, ms_real *bscat_ratio_out) {
    ...
    ms_real *bscat_air_out)
{
  ...
  for (i = 0; i < n; i++) {
    ms_real od = ext[i]*drange[i]; /* optical depth of layer */
    if (od > 0.0) {
    ...
    /* The single-scattering backscatter is the sum of the
       ...
       scatterers. */
    if (ext_air) {
      ms_real transmittance_2way = transmittance*transmittance;

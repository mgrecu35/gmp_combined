...
main(int argc, char **argv)
{
  ...
  do {
    strbuflen += MAX_CHARS;
    strbuf = realloc(strbuf, sizeof(char)*strbuflen);
    if (!strbuf) {
  ...
  if (output_adjoint) {
    ...
    bscat_AD = malloc(sizeof(ms_real)*instrument.nfov*m);
    if (separate_bscat_air) {
      bscat_air_AD = malloc(sizeof(ms_real)*instrument.nfov*m);
    }
    ...
    ext_AD = malloc(sizeof(ms_real)*n);
    ssa_AD = malloc(sizeof(ms_real)*n);
    g_AD = malloc(sizeof(ms_real)*n);
    radius_AD = malloc(sizeof(ms_real)*n);
    ext_bscat_ratio_AD = malloc(sizeof(ms_real)*n);
...
//  }
  ...
  if (calc_jacobian) {
    ms_real *jacobian_data
      = malloc(4*m*instrument.nfov*n*sizeof(ms_real));
    ms_real *jacobian_air_data = NULL;
    ms_real **d_bscat_d_ext = malloc(n*sizeof(ms_real*));
    ms_real **d_bscat_d_ssa = malloc(n*sizeof(ms_real*));
    ms_real **d_bscat_d_g = malloc(n*sizeof(ms_real*));
    ms_real **d_bscat_d_radius = malloc(n*sizeof(ms_real*));
    ms_real *d_bscat_d_ext_bscat_ratio
      = malloc(m*instrument.nfov*m*sizeof(ms_real));
    ...
    int *i_xy = malloc(m*instrument.nfov*sizeof(int));
    ...
    if (bscat_air) {
      jacobian_air_data 
	= malloc(4*m*instrument.nfov*n*sizeof(ms_real));
      d_bscat_air_d_ext = malloc(n*sizeof(ms_real*));
      d_bscat_air_d_ssa = malloc(n*sizeof(ms_real*));
      d_bscat_air_d_g = malloc(n*sizeof(ms_real*));
      d_bscat_air_d_radius = malloc(n*sizeof(ms_real*));
      for (i = 0; i < n; i++) {

...
calc_J(const gsl_vector* v, void* params)
{
  ...
  */
  ...
  for (i = 0; i < data->n; i++) {
    ...
    myJ += diff*diff / (BSCAT_PHOTON*data->bscat[i]);
    diff = data->bscat_air_fwd[i] - data->bscat_air[i];
    myJ += diff*diff / (BSCAT_PHOTON*data->bscat_air[i]);
...
calc_J_delJ(const gsl_vector* v, void* params, ms_real *myJ, gsl_vector* delJ)
{
  ...
  *myJ = calc_J(v, params); /* Needed to get bscat_fwd */
  ...
  for (i = 0; i < data->n; i++) {
    ...
    data->bscat_AD[i] = (data->bscat_fwd[i] - data->bscat[i]) 
      / (BSCAT_PHOTON*data->bscat[i]);
    data->bscat_air_AD[i] = (data->bscat_air_fwd[i] - data->bscat_air[i])
      / (BSCAT_PHOTON*data->bscat_air[i]);
  }
  //  fprintf(stderr,"!");
...
#ifdef USE_LOG
    gsl_vector_set(delJ, i, ext_AD[i]*ext[i]);
#else
...
main(int argc, char** argv)
{
  ...
  ext[0] = 1.0e-6;
  ext[n/4] = ext[n/4+1] = 1e-3;

  ext[n/2] = ext[n/2+1] = 1e-3;
  ...
  */
  ...
  do {
    ...
    if (minimizer->f < 0.1/n) {
      status = GSL_CONTINUE+1;
    }
    printf(">>> %d %g\n", iter, minimizer->f);
    for (i = 0; i < n; i++) {
      printf("%2d %10.3g %10.3g %10.3g %10.3g %10.3g %10.3g\n", i, ext[i], 
#ifdef USE_LOG
	     exp(gsl_vector_get(minimizer->x, i)),
#else
	     gsl_vector_get(minimizer->x, i),
#endif
	     bscat[i], bscat_fwd[i], bscat_air[i], bscat_air_fwd[i]);
    }
  }  
